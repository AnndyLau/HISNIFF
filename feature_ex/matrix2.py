...
# 计算每个类别的准确率
data_path = '/home/cz/data/final_data/datatxt_b7_1/data18-51-zhong-head1level'
f = open(data_path+'-matrix.txt',mode = 'r',encoding='utf-8')
s=f.read()
f.close()
confusion_matrix2 = np.array(eval(s))#np.zeros((47,47))
confusion_matrix = confusion_matrix - confusion_matrix2
#class_accuracy = confusion_matrix / confusion_matrix.sum(axis=1)[:, None]

# 绘制混淆矩阵图像
# 要想改变颜色，修改cmap参数，红色：plt.cm.Reds
plt.figure(figsize=(15,15), dpi=80)
plt.imshow(confusion_matrix, cmap=plt.cm.coolwarm, norm=plt.Normalize(-np.max(confusion_matrix), np.max(confusion_matrix)))
...
for i, j in itertools.product(range(confusion_matrix.shape[0]), range(confusion_matrix.shape[1])):
    ...
    plt.text(j, i, value, verticalalignment='center',horizontalalignment="center", color="black" if value == 0 else 'white' )
