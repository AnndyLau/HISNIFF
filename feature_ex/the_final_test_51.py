...
import os
from feature_file.outfeature import out_feature,out_feature_18,image_process,guiyihua,guiyihua2,out_feature_51
...
# class_49_2 = ['Condylura cristata', 'Desmana moschata', 'Dymecodon pilirostris','Euroscaptor grandis', 
#             'Euroscaptor klossi', 'Euroscaptor kuznetsovi', 'Euroscaptor longirostris', 'Euroscaptor malayana', 'Euroscaptor micrura', 
#             'Euroscaptor orlovi', 'Euroscaptor parvidens', 'Galemys pyrenaicus', 'Mogera etigo', 'Mogera hainana', 'Mogera imaizumii', 
#             'Mogera insularis', 'Mogera kanoana', 'Mogera latochei', 'Mogera robusta', 'Mogera tokudae', 'Mogera wogura', 'Neurotrichus gibbsii', 
#             'Orescaptor mizura', 'Parascalops breveri', 'Parascaptor Parascaptor sp1', 'Parascaptor Parascaptor sp2', 'Parascaptor leucura', 'Scalopus aquaticus', 
...
#             'Scaptonyx fusicaudus', 'Scaptonyx sp3', 'Talpa altaica', 'Talpa aquitania', 'Talpa caeca', 'Talpa europaea', 'Talpa levantis', 'Talpa romana', 
#             'Talpa talyschensis', 'Uropsilus andersoni', 'Uropsilus gracilis', 'Uropsilus investigator', 'Uropsilus nivatus', 'Uropsilus soricipes', 'Urotrichus talpoides']
# class_49_3 = ['Condylura cristata', 'Desmana moschata', 'Dymecodon pilirostris', 'Euroscaptor Euroscaptor sp1', 'Euroscaptor grandis', 
#             'Euroscaptor klossi', 'Euroscaptor kuznetsovi', 'Euroscaptor longirostris', 'Euroscaptor malayana', 'Euroscaptor micrura', 
...
print('正在读取归一化工具')
class_num = '1'
teeth_tool = joblib.load('./scaler_dir_b7'+class_num+'/51_teeth.pkl')
head_tool = joblib.load('./scaler_dir_b7'+class_num+'/51_head.pkl')
...
server_name = 'data51-combine_od'
matrix = np.zeros((len(class_49),len(class_49)))
matrix2 = np.zeros((len(class_18),(len(class_18))))
...
f6 = open(txt_path+server_name+'-shu-matrix.txt', mode = 'w')
print(shu_tool)
test_num = 0
for shu in class_49:
    zhong_acc = 0
    ...
    for img in os.listdir(head_imagedir_path):#头骨图片名与牙齿图片名一样
        ...
        with torch.no_grad():
            ...
            matrix2[class_18.index(r_shu)][class_18.index(shu_result)]+=1
            if(shu_result==r_shu):
                shu_r_num+=1
                pass
                #continue
        ...
        head_feature_shu = out_feature_51(teeth_image,head_image,'head',class_num.split('_')[0])
        teeth_feature_shu = out_feature_51(teeth_image,head_image,'teeth',class_num.split('_')[0])
        head_feature_shu = process(head_feature_shu)
        teeth_feature_shu = process(teeth_feature_shu)
        concat_feature_shu = guiyihua(teeth_feature_shu,head_feature_shu,shu_tool[0][0],shu_tool[0][1])
        net_51 = torch.load('./feature_file/weights-b7_'+class_num+'/51/concat_1:1/best_model.pkl',map_location=device)
        net_51.to(device)
        net_51.eval()
        zhong_output = net_51(concat_feature_shu)
        zhong_result = zhong_output.argmax(1)
        zhong_result = str(zhong_result).split('[')[1].split(']')[0]
        zhong_result = class_49[int(zhong_result)]
        r_zhong = shu
...
f6.writelines(str(matrix2.tolist()))
print('18属分类准确率为{:.2f}%'.format(shu_r_num/test_num*100))
print('一级融合网络准确率为{:.2f}%'.format(r_num/test_num*100))
f.close()
f2.close()
f3.close()
f4.close()
