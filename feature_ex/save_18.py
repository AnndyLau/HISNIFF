...
def out_feature(teeth_img,head_img,numclass,num,torh):
    ...
    tooth_net = "../teeth_5zhe_data/teeth_data_"+class_num+"/weights-b7-teeth/EfficientNet_50_"+str(num)+"/best_network_eb4.pth"
    ...
    whole_net = "../head_5zhe_data/head_data_"+class_num+"/weights-b7-head/EfficientNet_50_"+str(num)+"/best_network_eb4.pth"
...
process2.to(device)
#process.eval()
shu_list = ['18','51']#['Scapanus','Mogera','Scaptonyx','Uropsilus','Euroscaptor','Parascaptor','Talpa']
zhong_list = [18,51]#[4,9,4,5,8,3,5]
# 创建朴素贝叶斯分类器
model = MultinomialNB(alpha=1.0)
#model = SVC()
class_num='1'
server_name = 'b7_'+class_num#+'_aug'
for zhong,shu in enumerate(shu_list):
    print(shu)
    for torh in ['teeth','head']:#分别提取头骨牙齿信息
        best_acc=0
        print(torh)
        if(torh == 'teeth'):process=process1#根据头骨或牙齿选择处理方式
        else:process=process2
        process.eval()
        file_path = './feature_file/'+server_name+'/'+shu+'_noaug/'+torh+'_data'
        if os.path.exists(file_path) is False:
            os.makedirs(file_path)
        print(file_path.split('/')[-1].split('_')[0])
        datapath = '../head_5zhe_data/head_data_'+class_num+'/head_'+shu+'_r_p'
        teeth_datapath = '../teeth_5zhe_data/teeth_data_'+class_num+'/teeth_'+shu+'_r_p_noaug'
        feature_test_list = []
        y_test_list = []
        ty_test_list = []
        ...
        with tqdm(zip(test_dataloader,teeth_test_dataloader),desc="正在提取测试集特征",leave=False,colour='green') as test_bar:
            ...
            y_train_list = []
            ty_train_list = []
        for epoch in range(1):#后期数据增强修改轮数
            ...
            with tqdm(zip(train_dataloader,teeth_train_dataloader),desc="第{}轮，正在提取训练集特征".format(epoch),colour='green') as train_bar:
