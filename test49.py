...
server_name = 'data18-51-zhong-'+lei+'1level_od'
...
shu_49_path = './'+lei+'_5zhe_data/'+lei+'_data_'+class_num+'/weights-'+server_name+'/EfficientNet_50_51/best_network_eb4.pth'
shu_acc = 0
acc_count_all =0
for classname in class_49:#class_49:#51种
    ...
    for img in image_data:#图片
        ...
        matrix2[class_18.index(r_shu)][class_18.index(shu)]+=1
        if(r_shu==shu):
            #print('判断正确')
            shu_acc+=1
            pass
        ...
        #net = EfficientNet.from_name('efficientnet-b7').to(device)
        num_ftrs = net._fc.in_features
        net._fc = nn.Linear(num_ftrs, len(class_49))
        #net._fc = nn.Linear(num_ftrs, 2500)
        net.load_state_dict(torch.load(shu_49_path,map_location=device))
        net.to(device)
        with torch.no_grad():
            output = net(image)
        result = output.argmax(1)
        result1 = str(result).split('[')[1].split(']')[0]#对应种分类器中的哪一种
        zhong =  class_49[eval(result1)]
        r_zhong = classname
